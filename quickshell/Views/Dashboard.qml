...
Rectangle {
    ...
    // Dynamic Dimensions
    width: canvas.dashboardOpen ? Style.dashboard.width : Style.widgets.width
    height: canvas.dashboardOpen ? Style.dashboard.height : Style.widgets.height
    ...
    // Click to toggle
    // TODO: Even when disabled on dashboard open, cursor remains pointing hand
    MouseArea {
        anchors.fill: parent
        enabled: !canvas.dashboardOpen // Only active when minimized
        onClicked: canvas.dashboardOpen = true
        hoverEnabled: true
        cursorShape: canvas.dashboardOpen ?  Qt.ArrowCursor : Qt.PointingHandCursor
    }
